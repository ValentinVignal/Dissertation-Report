...
I spend some time to create songs and arrange them. And finally, I enjoy jamming with band and improvise.
...
Therefor, I try in this work to combine combine my understanding of music, the knowledge so far on music generation and the capability to handle complex data with several modalities.
...
The choice I have made for this project was mostly relying on my musical instinct.
I tried to stay as close as possible as my musical thoughts in the way I constructed a model, process the data et caetera.
...
They achieve this perfomance by using a Transformer with self attention (equation \ref{eq:relative-attention}).
...
At the start of my dissertation, I tried another project which haw to create music with a waveform representation.
...
More precisely, their model is a shallow network able to learn complex function containing multiplication, sinus and cosine functions, and it is able to extrapolate them.
Those very usual operations processed by most of the VST to create musical sounds.
...
Contrariwise, a musical waveform contains  enormous number of sine and cosine components. 
...
Train a model and generate music is nice.
...
\begin{itemize}
    \item Allows the user to plays music with the instrument of his choice
    \item Can load a trained model
    \item Allows the user to choose what part he wants to play in the \textit{"band"}
    \item Allows the user to choose what parts from the \textit{"band"} he wants to keep
...
Therefor the model is called only one time per measure which is not to frequente to allow the application to be real time.
...
The reason is because every number or setting can be considered as a hyper-parameter and, most of time, there is no proof and rule to guide use to choose the best value they should have.
The only help the researcher can have is his intuition and his understanding on how an specific parameter will influence a specific process.
...
Since the number of hyper-parameters in my very high, (at some point, I was trying to tune over 20 parameters), I implemented a script to do a Bayesian search over them.
To do so I used the library scikit-optimize \cite{noauthor_scikit-optimize_nodate}.
The main advantages of a Bayesian optimization over a grid or a random search are:
\begin{itemize}
